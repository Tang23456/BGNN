...
plt.plot(data_mvg['mean_fpr'], data_mvg['mean_tprs'], label=f'BGNN (AUC = {data_mvg["mean_auc"]:.2f})', lw=line_width, color=colors[0])
...
# with open('./compare_modle/compare_data/SAELGMDA_change/roc_data.pkl', 'rb') as f:
#     roc_data = pickle.load(f)
...
# 模型1: BGNN
plt.plot(data_mvg['mean_recall'], data_mvg['mean_precisions'], label=f'BGNN (AUPR = {data_mvg["mean_average_precision"]:.2f})', lw=line_width, color=colors[0])
...
# 模型3: SAELGMDA
# with open('./compare_modle/compare_data/SAELGMDA_change/pr_data.pkl', 'rb') as f:
#     pr_data = pickle.load(f)
