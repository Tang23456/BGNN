...
def Cross_Validation(data, label,dataset_name, k=5, cv=3):
    all_performance = []
    # data_select, label = my_change_data(data, label)
    train_index, test_index = Cross_Validation_with_KFold_MY(data, label, k=5)
    # train_index, test_index = kfold(data=data, k=k, cv=cv)
...
def Cross_Validation_with_KFold_MY(data, labels, k=5):
    """
    使用 5 折交叉验证对平衡且打乱后的数据进行训练和测试集划分

    参数:
    - data: 输入数据 (numpy 数组, 形状为 [n_samples, n_features])
    - labels: 输入标签 (numpy 数组, 形状为 [n_samples, 1])
    - k: k-fold 中的 k 值，默认为5

    返回:
    - 分别返回训练集索引和测试集索引
    """
    # 调用 my_change_data 进行数据平衡和打乱
    X_train, y_train = my_change_data(data, labels)
    ...
    for train_index, test_index in kf.split(X_train):
        train_indices.append(X_train[train_index])
        test_indices.append(X_train[test_index])
...
def my_change_data(data, labels):
    """
    函数将标签为1的样本和标签为0的样本按要求平衡，并随机打乱后返回训练数据和标签。
    ...
    # 从标签为0的索引中随机选择与标签为1数量相同的样本
    num_1 = len(indices_1)
    indices_0_selected = np.random.choice(indices_0, num_1, replace=False)

    # 合并标签为1的索引和选出的标签为0的索引
    selected_indices = np.concatenate([indices_1, indices_0_selected])

    # 打乱选出的索引
    np.random.shuffle(selected_indices)

    # 根据打乱后的索引提取数据
    out_index = selected_indices
    out_label = labels[selected_indices]

    return out_index, out_label
