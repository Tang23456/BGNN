...
global mean_tpr
global mean_recall
global mean_precision
global mean_fpr

global mean_tpr_Only_pos
global mean_recall_Only_pos
global mean_precision_Only_pos
global mean_fpr_Only_pos

global mean_tpr_Only_neg
global mean_recall_Only_neg
global mean_precision_Only_neg
global mean_fpr_Only_neg

global mean_tpr_A_neg
global mean_recall_A_neg
global mean_precision_A_neg
global mean_fpr_A_neg

global mean_tpr_A_neg_change
global mean_recall_A_neg_change
global mean_precision_A_neg_change
global mean_fpr_A_neg_change
...
sk_precisions_A_neg = []
sk_recalls_A_neg = []
sk_average_precisions_A_neg = []
sk_fpr_A_neg = []


sk_tprs_A_neg_change = []
sk_aucs_A_neg_change = []
sk_precisions_A_neg_change = []
sk_recalls_A_neg_change = []
sk_average_precisions_A_neg_change = []
sk_fpr_A_neg_change = []
...
def test_main():
    ...
    global sk_tprs
    global sk_aucs
    global sk_precisions
    global sk_recalls
    global sk_average_precisions
    global sk_fpr
    tprs = []
    aucs = []
    test_label_score = {}
...
def test_Only_neg():
    out = []
    mean_fpr = np.linspace(0, 1, 100)
    global sk_tprs_Only_neg
    global sk_aucs_Only_neg
    global sk_precisions_Only_neg
    global sk_recalls_Only_neg
    global sk_average_precisions_Only_neg
    global sk_fpr_Only_neg
    test_label_score = {}
    tprs = []
    aucs = []
    iter_ = 0
...
def test_A_neg():
    out = []
    mean_fpr = np.linspace(0, 1, 100)
    global sk_tprs_A_neg
    global sk_aucs_A_neg
    global sk_precisions_A_neg
    global sk_recalls_A_neg
    global sk_average_precisions_A_neg
    global sk_fpr_A_neg
    test_label_score = {}
    iter_ = 0
    tprs = []
    ...
    for cl, (train_index, test_index) in enumerate(kf.split(samples)):  # 循环每一折的训练和测试集
        ...
        with torch.no_grad():
            ...
            fpr_temp, tpr_temp, _ = roc_curve(test_labels, perdcit_score)
            roc_auc = auc(fpr_temp, tpr_temp)
            sk_fpr_A_neg.append(fpr_temp)
            sk_tprs_A_neg.append(tpr_temp)
            sk_aucs_A_neg.append(roc_auc)

            precision_temp, recall_temp, _ = precision_recall_curve(test_labels, perdcit_score)
            average_precision = average_precision_score(test_labels, perdcit_score)
            sk_precisions_A_neg.append(precision_temp)
            sk_recalls_A_neg.append(recall_temp)
            sk_average_precisions_A_neg.append(average_precision)
    ...
    mean_fpr_A_neg = np.linspace(0, 1, 100)
    mean_recall_A_neg = np.linspace(0, 1, 100)
    tprs = []
    precisions = []
    for fpr_temp, tpr_temp in zip(sk_fpr_A_neg, sk_tprs_A_neg):
        interp_tpr = np.interp(mean_fpr_A_neg, fpr_temp, tpr_temp)
        interp_tpr[0] = 0.0  # 确保曲线从 0 开始
        tprs.append(interp_tpr)
    mean_tpr_A_neg = np.mean(tprs, axis=0)

    mean_tpr_A_neg[-1] = 1.0  # 确保曲线以 1 结束

    for recall_temp, precision_temp in zip(sk_recalls_A_neg, sk_precisions_A_neg):
        interp_precision = np.interp(mean_recall_A_neg, recall_temp[::-1], precision_temp[::-1])
        precisions.append(interp_precision)

    mean_precision_A_neg = np.mean(precisions, axis=0)

    sk_aucs_A_neg = np.mean(sk_aucs_A_neg)
    sk_average_precisions_A_neg = np.mean(sk_average_precisions_A_neg)
    # 删除模型变量
    del model
    torch.cuda.empty_cache()  # 如果使用 GPU，清除显存缓存
    gc.collect()  # 手动调用垃圾回收器
...
def test_Only_pos():
    out = []
    mean_fpr = np.linspace(0, 1, 100)
    global sk_tprs_Only_pos
    global sk_aucs_Only_pos
    global sk_precisions_Only_pos
    global sk_recalls_Only_pos
    global sk_average_precisions_Only_pos
    global sk_fpr_Only_pos
    test_label_score = {}
    tprs = []
    aucs = []
    iter_ = 0
...
def test_A_neg_change():
    out = []
    mean_fpr = np.linspace(0, 1, 100)
    global sk_tprs_A_neg_change
    global sk_aucs_A_neg_change
    global sk_precisions_A_neg_change
    global sk_recalls_A_neg_change
    global sk_average_precisions_A_neg_change
    global sk_fpr_A_neg_change
    test_label_score = {}
    tprs = []
    aucs = []
    iter_ = 0
    ...
    for cl, (train_index, test_index) in enumerate(kf.split(samples)):  # 循环每一折的训练和测试集
        ...
        with torch.no_grad():
            ...
            fpr_temp, tpr_temp, _ = roc_curve(test_labels, perdcit_score)
            roc_auc = auc(fpr_temp, tpr_temp)
            sk_fpr_A_neg_change.append(fpr_temp)
            sk_tprs_A_neg_change.append(tpr_temp)
            sk_aucs_A_neg_change.append(roc_auc)

            precision_temp, recall_temp, _ = precision_recall_curve(test_labels, perdcit_score)
            average_precision = average_precision_score(test_labels, perdcit_score)
            sk_precisions_A_neg_change.append(precision_temp)
            sk_recalls_A_neg_change.append(recall_temp)
            sk_average_precisions_A_neg_change.append(average_precision)
    ...
    mean_fpr_A_neg_change = np.linspace(0, 1, 100)
    mean_recall_A_neg_change = np.linspace(0, 1, 100)
    tprs = []
    precisions = []
    for fpr_temp, tpr_temp in zip(sk_fpr_A_neg_change, sk_tprs_A_neg_change):
        interp_tpr = np.interp(mean_fpr_A_neg_change, fpr_temp, tpr_temp)
        interp_tpr[0] = 0.0  # 确保曲线从 0 开始
        tprs.append(interp_tpr)
    mean_tpr_A_neg_change = np.mean(tprs, axis=0)

    mean_tpr_A_neg_change[-1] = 1.0  # 确保曲线以 1 结束

    for recall_temp, precision_temp in zip(sk_recalls_A_neg_change, sk_precisions_A_neg_change):
        interp_precision = np.interp(mean_recall_A_neg_change, recall_temp[::-1], precision_temp[::-1])
        precisions.append(interp_precision)

    mean_precision_A_neg_change = np.mean(precisions, axis=0)

    sk_aucs_A_neg_change = np.mean(sk_aucs_A_neg_change)
    sk_average_precisions_A_neg_change = np.mean(sk_average_precisions_A_neg_change)
...
# 数据定义（假设这些变量已经正确计算）
model_labels = ['BGNN', 'BGNN_Only_pos', 'BGNN_Only_neg', 'BGNN_A_neg', 'BGNN_A_neg_change']

model_precisions = [mean_precision, mean_precision_Only_pos, mean_precision_Only_neg, mean_precision_A_neg, mean_precision_A_neg_change]
model_recalls = [mean_recall, mean_recall_Only_pos, mean_recall_Only_neg, mean_recall_A_neg, mean_recall_A_neg_change]
model_auprs = [sk_average_precisions, sk_average_precisions_Only_pos, sk_average_precisions_Only_neg, sk_average_precisions_A_neg, sk_average_precisions_A_neg_change]

model_tprs = [mean_tpr, mean_tpr_Only_pos, mean_tpr_Only_neg, mean_tpr_A_neg, mean_tpr_A_neg_change]
model_fprs = [mean_fpr, mean_fpr_Only_pos, mean_fpr_Only_neg, mean_fpr_A_neg, mean_fpr_A_neg_change]
model_aucs = [sk_aucs, sk_aucs_Only_pos, sk_aucs_Only_neg, sk_aucs_A_neg, sk_aucs_A_neg_change]
